...
const AdminNav = () => {
  ...
  useEffect(() => {
    ...
    for (let x in nav__links) {
      const saved_path = nav__links[x];
      if (p.startsWith(saved_path)) setLoc(x + 1);
    }
          ...
          <>
            {nav__links.map((nv, idx) => {
              return loc === idx + 1 ? (
                ...
                >
                  <Link href={`/${nv.path}`}>{nv.name}</Link>
                </p>
              ) : (
                <p
